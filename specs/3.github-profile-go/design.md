...
### Terraform 文件（对照参考 `infra/`）

| 文件 | 内容 |
| ---- | ---- |
| network.tf | VPC 10.0.0.0/16、2 AZ、公网/私网子网、NAT、IGW、路由表 |
| security.tf | 安全组链：ALB←BFF、Go←ALB、Aurora←Go、Secrets |
| ecr.tf | profile-go 仓库 |
| alb.tf | 内网 ALB + listener + `/profile/*` 目标组 |
| cloudmap.tf | 私有命名空间（预留东西向） |
| ecs.tf / ecs-profile.tf | ECS 集群 + Fargate 服务 + 任务定义（注入 Secrets） |
| iam-ecs.tf | 任务执行角色 / 任务角色 |
| bff.tf | Lambda BFF + API Gateway(HTTP) + VPC 配置 |
| rds.tf | **引用现有 Aurora**（data source，不新建）；加 Go SG 入站放行 |
| variables/outputs/providers/versions.tf | 常规 |
...
| 角色 | 职责 |
| ---- | ---- |
| Cloudflare | 每 PR 一个预览前端（PR preview URL），指向共享 test 后端 |
| CodeBuild（VPC 内） | buildspec：分支/DB 守卫 → docker build Go → 推 ECR `:pr-N` → 有迁移则**对共享 dev 库仅校验（不真改）** → 起/更新 `pr-N` 的 ECS 服务 |
| IAM | CodeBuild 最小权限：ECR push、ECS 部署、创建 ENI 进 VPC、读 Secrets、写日志 |
...
```
开/更新 PR → CodeBuild(pr-N): build→push→迁移校验→部署 ECS pr-N
           → Cloudflare 出 pr-N 预览 URL（指向共享 test API）
...
- **成本**：NAT/ALB/RDS/Fargate 按小时计费。约定「学完即拆」：`terraform workspace select test && terraform destroy`；只开一套环境；配 Budgets 1 美元告警。
